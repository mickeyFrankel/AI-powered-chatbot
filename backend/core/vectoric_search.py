...
class VectorDBQASystem:
    ...
    def comprehensive_search(self, query: str, max_results: int = 20, show_reasoning: bool = True) -> dict:
        ...
        reasoning_steps.append({
            ...
            'details': 'Level 1: Exact name → Level 2: Phrase match → Level 3: All words → Level 4: Partial → Level 5: Fuzzy → Level 6: Semantic'
        })
        ...
        reasoning_steps.append({
            ...
            'result': f'✅ Found {len(level_results["exact_name"])} exact matches' if level_results['exact_name'] else '❌ No exact name matches',
        ...
        reasoning_steps.append({
            ...
            'result': f'✅ Found {len(level_results["phrase_match"])} phrase matches' if level_results['phrase_match'] else '❌ No phrase matches',
        ...
        reasoning_steps.append({
            ...
            'result': f'✅ Found {len(level_results["all_words"])} contacts with all words' if level_results['all_words'] else '❌ No contacts with all words',
        ...
        reasoning_steps.append({
            ...
            'result': f'✅ Found {len(level_results["partial"])} partial matches' if level_results['partial'] else '❌ No partial matches',
        ...
        reasoning_steps.append({
            ...
            'result': f'✅ Found {len(level_results["fuzzy"])} fuzzy matches' if level_results['fuzzy'] else '❌ No fuzzy matches',
        ...
        reasoning_steps.append({
            ...
            'result': f'✅ Found {semantic_count} semantic matches' if semantic_count > 0 else '❌ No semantic matches',
        ...
        reasoning_steps.append({
            ...
            'result': f'✅ {len(all_results)} total matches found',
...
class AdvancedVectorDBQASystem(VectorDBQASystem):
    ...
    def _create_tools(self):
        ...
        def smart_search(query: str, max_results: int = 20) -> dict:
            """🌟 PRIMARY SEARCH TOOL - Use this for ALL person/contact searches!
            ...
            Automatically:
            - Translates relationship queries ("מי שילדה אותי" → "אמא")
            - Handles typos and variations
            - Returns ALL relevant results ranked by confidence
            - Shows why each result matched
            
            Use for:
            - Individual person searches ("דוד", "David")
            - Relationship queries ("אמא שלי", "my uncle")
            - Names with typos ("curtain" → finds "curtains")
    ...
    def _create_prompt(self):
        ...
        return ChatPromptTemplate.from_messages([
            ("system", """You are a contact search assistant for Hebrew/English queries.
